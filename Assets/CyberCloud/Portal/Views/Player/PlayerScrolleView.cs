...
public class PlayerScrolleView : MonoBehaviour
{
    #region data
    ...
    private List<GalleryImageItem> mItemList = new List<GalleryImageItem>();
    ...
    public void InitBase()
    {
        ...
        mPhotoModelList = data.PhotoList;

        int have = mPhotoModelList.Count;
        int index = data.GetIndexByID(CachePhotoData.Instance.CurrentPhotoIndex);
    ...
    private void InitIconData()
    {
        ...
        {
            mItemList[i].Init(mPhotoModelList[i]);
        }
    ...
    private void AddIconData()
    {
        ...
        {
            mItemList.Add(CreateImageItem(i));
            ...
            //设置贴图
            Texture2D icon = CachePhotoData.Instance.GetIconTexture(mPhotoModelList[i].MID);

            if (icon == null)
            {
                //理论上这时候的图片都需要下载
                DataLoader.Instance.LoadTexture(mPhotoModelList[i]);
            }
            else
            {
                mItemList[i].GetComponent<UITexture>().mainTexture = icon;
            }
    ...
    private GalleryImageItem CreateImageItem(int index)
    {
        ...
        GalleryImageItem item = UnityTools.CreateComptent<GalleryImageItem>(mIconTemplate, transform, Vector3.zero, Quaternion.Euler(Vector3.zero), 0.8f * Vector3.one, "Item_" + index);
        Transform tran = item.transform;
        GameObject go = item.gameObject;
        //go.transform.parent = this.gameObject.transform;
        //go.transform.localScale = this.gameObject.transform.localScale;

        tran.localRotation = CalculateItemRotationByIndex(index);
        tran.localPosition = CalculateItemPositionByAngleY(tran.localRotation.eulerAngles.y);
        //go.GetComponent<PanelIconButton>().OriginPos = tran.localPosition;

        int indexInPage = index % mColumnsPerPage;
        int nowPage = (index / mColumnsPerPage) + 1;
        if (nowPage != mCurrentPage)
        {
            item.mTexture.alpha = 0.0f;
            go.SetActive(false);
        }
        return item;
    }
    ...
    private void TransitPage(PageDirection direction, bool isCallback = false)
    {
        ...
        //    }
            ...
            foreach (var obj in mItemList)
            {
                ItemMove(obj.cachegameobject, direction);
            }
    ...
    private void MakeAllItemsDisabled()
    {
        foreach (var obj in mItemList)
        {
            obj.cachegameobject.GetComponent<BoxCollider>().enabled = false;
        }
    }
    ...
    private void ClearIconData()
    {
        ...
        if (mItemList != null)
        {
            ...
            {
                GameObject itemToDelete = mItemList[0].gameObject;
