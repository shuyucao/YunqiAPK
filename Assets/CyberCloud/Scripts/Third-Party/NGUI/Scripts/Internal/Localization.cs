...
public static class Localization
{
	...
	static public string language
	{
		get
		{
			if (string.IsNullOrEmpty(mLanguage))
			{
               ...
               // Debug.LogError("ucvr loadlocationToDictionary0 load languge=" + lan[0]);
		...
		set
		{
			...
			{
				mLanguage = value;
                MyTools.PrintDebugLog("ucvr loadlocationToDictionary1 set mLanguage="+value);
                LoadAndSelect(value);
    ...
    /// <summary>
	/// Get Current Language's font  2016.11.28
    /// </summary>
	static public Font CurrentFont
	{
		get{
			Font result=new Font();
            if (CyberLanguage.Instance != null)
                result = CyberLanguage.Instance.Arial;
       
			return result;
		}
	}
	/// <summary>
	///  Dynamic set label's text and font  2016.11.28
	/// </summary>
	/// <returns>The text.</returns>
	/// <param name="lbl">Lbl.</param>
	/// <param name="key">Key.</param>
	public static void SetText(UILabel lbl,string key)
	{
		if (lbl != null) {
			lbl.trueTypeFont=CurrentFont;
	...
	static bool LoadAndSelect (string value)
	{
		...
		{
            MyTools.PrintDebugLog("ucvr loadlocationToDictionary2");
    ...
    /// 从语言包里找到使用语言的字典索引mLanguageIndex
    /// </summary>
    ...
    static bool SelectLanguage (string language)
	{
		...
		string[] keys;
        //获取 第一行 KEY,zh,en,ja,fr,de,it,es 所有语言对应的索引
    ...
    /// 帮助函数从CSV文件添加单行到本地化列表。
    /// values 为某一行的数据存储了 key 中文 英文 日文 。。。 values【0】是中文
    /// mDictionary的key为values【0】，value值为key对应的各国语言
    /// </summary>
    static void AddCSV (BetterList<string> values)
	{
		...
		try
		{
            //Debug.LogError("values[0]  '" + values[0] + "' values[1] :" + values[1]);
	...
	static public void Set (string languageName, Dictionary<string, string> dictionary)
	{
        ...
        MyTools.PrintDebugLogError("ucvr setlanguage " + mLanguage);
        //PlayerPrefs.SetString("Language", mLanguage);
	...
	static public string Get (string key)
	{
        ...
        //string l= PlayerPrefs.GetString("Language", "en");
