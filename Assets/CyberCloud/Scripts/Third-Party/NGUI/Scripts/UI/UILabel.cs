...
public class UILabel : UIWidget
{
	...
	public bool IsClampByMask = false;
	public string text {
		get {
			return mText;
		}
		set {
			...
			if (string.IsNullOrEmpty (value)) {
				...
				if (IsClampByMask) {
					string maskString="...";
					string result="";
					int offset = CalculateOffsetToFit_Oscar (mText);
					if (offset > 0) {
						string tempString = "";
						for (int i=0; i<=offset; i++) {
							tempString += mText [i];
						}
						tempString = maskString + tempString;
						offset = CalculateOffsetToFit_Oscar (tempString);
						for (int i=maskString.Length; i<=offset;i++) {
							result += tempString [i];
						}
						result +=maskString;
						mText = result;
	...
	{
		base.OnStart();
        // Add to change font start 2016.11.28
        mmStarted = true;
        OnLocalize();
        // Add to change font end 2016.11.28
        // Legacy support
        if (mLineWidth > 0f)
		{
	...
	/// <summary>
	/// Calculates how many char can show in current lable.
	/// add by oscar
	/// </summary>
	/// <returns>can show char count</returns>
	...
	public int CalculateOffsetToFit_Oscar (string text)
	{
		...
		NGUIText.encoding = false;
		NGUIText.symbolStyle = NGUIText.SymbolStyle.None;
		int offset = NGUIText.CalculateOffsetToFit_Oscar(text);
		NGUIText.bitmapFont = null;
		#if DYNAMIC_FONT
		NGUIText.dynamicFont = null;
		#endif
	...
	void OnLocalize ()
	{
		if(Localization.CurrentFont!=null)
		this.trueTypeFont = Localization.CurrentFont;
	}
}
