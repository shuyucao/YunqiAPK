//----------------------------------------------
//            NGUI: Extend
// Copyright © 2016 Xavier Entertainment
//----------------------------------------------
...
[AddComponentMenu("NGUI/Interaction/UISpecialGrid")]
public class UISpecialGrid : UIWidgetContainer
{
    ...
    public float cellZ = 10;

    public float cellRoa = 10;
    ...
    //    {
    //        Transform t = list[0];
    //        Game.DestroyImmediate(t.gameObject);
    //        list.RemoveAt(0);
    //    }
    //}
    ...
    protected void ResetPosition(List<Transform> list)
    {
        ...
        if (pivot != UIWidget.Pivot.TopLeft)
        {
            ...
            //ResetRoatation
            List<float> z_list = StyleUtils.GetCalculatedZ(cellZ, myTrans.childCount);
            List<Quaternion> roatate_list = StyleUtils.GetCalculatedRoatation(cellRoa, myTrans.childCount);
            for (int i = 0; i < myTrans.childCount; ++i)
            {
                Transform t = myTrans.GetChild(i);
                Vector3 pos = t.localPosition;
                pos.z = -z_list[i];
                t.localRotation = roatate_list[i];
                t.localPosition = pos;
            }
        }
    }
